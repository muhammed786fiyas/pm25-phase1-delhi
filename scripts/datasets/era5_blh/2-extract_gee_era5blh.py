...
def extract_cell_chunk(era5_chunk, cell_lat, cell_lon):
    point = ee.Geometry.Point(cell_lon, cell_lat)
    region_data = era5_chunk.getRegion(point, scale=1000).getInfo()
...
def main():
    ...
    for i in range(len(unique_cells)):
        ...
        for chunk_start, chunk_end in chunks:
            era5_chunk = era5_base.filterDate(chunk_start, chunk_end)
            chunk_df = extract_cell_chunk(era5_chunk, cell["cell_lat"], cell["cell_lon"])
    ...
    out_path = os.path.join(args.outdir, "era5_blh_raw.csv")
